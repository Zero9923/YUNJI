// settings.js - 允纯机核心模块：极致扁平 / 字体预设保存 / 字号强制生效
...
const SettingsApp = {
    init() {
        const defaults = {
            ...
            font_size: 15,
            temp_font_data: null, // 临时上传预览用
            current_font_data: null, // 当前应用的
            font_presets: [], // {name, data}
    ...
    applyGlobalStyles() {
        ...
        const fontData = this.config.current_font_data;
        const fontFace = fontData 
            ? `@font-face { font-family: 'YCJCustom'; src: url("${fontData}"); font-display: swap; }` 
            : '';
        ...
        styleTag.innerHTML = `
            ...
            * { 
            ...
            .s-acc-icon, .p-tag .x { font-size: 18px !important; }
    ...
    render() {
        ...
        const style = `
            <style>
                ...
                .s-header h1 { margin: 0; font-weight: 300; letter-spacing: 1px; color: #1a1a1a; }
                ...
                .s-input, .s-select {
                    ...
                    padding: 10px 0; color: #333; outline: none; border-radius: 0; width: 100%;
                }
                ...
                .s-btn-flat { padding: 10px 15px; font-size: 11px; border: 1px solid #EEE; color: #888; cursor: pointer; background: #fff; letter-spacing: 1px; }
                ...
                .s-footer { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; }
                ...
                <div class="s-content">
                    ...
                    <div class="s-accordion" onclick="SettingsApp.toggle(this)">
                        ...
                        <div class="s-acc-content" onclick="event.stopPropagation()">
                            ...
                            <div class="s-row"><div class="s-label">MODEL / 模型</div><select id="api-model" class="s-select">${this.renderOptions(this.config.model_list, this.config.model)}</select>
                            <div class="s-btn-group"><div class="s-btn-flat" onclick="SettingsApp.fetchModels('api')">FETCH / 获取列表</div></div></div>
                            <div class="p-list" id="api-presets"></div>
                            <div class="s-btn-group"><div class="s-btn-flat s-btn-dark" onclick="SettingsApp.saveCore('api')">SAVE / 保存</div><div class="s-btn-flat" onclick="SettingsApp.savePreset('api')">AS PRESET / 存为预设</div></div>
                        </div>
                    </div>
                    ...
                    <div class="s-accordion" onclick="SettingsApp.toggle(this)">
                        ...
                        <div class="s-acc-content" onclick="event.stopPropagation()">
                            ...
                            <div class="s-row">
                                <div class="s-label">Font Import / 导入字体</div>
                                <div class="s-btn-group">
                                    <div class="s-btn-flat" onclick="document.getElementById('f-picker').click()">UPLOAD / 上传文件</div>
                                    <div class="s-btn-flat s-btn-dark" onclick="SettingsApp.saveFontPreset()">SAVE PRESET / 保存为预设</div>
                                </div>
                            </div>
                    ...
                    <div class="s-accordion" onclick="SettingsApp.toggle(this)">
                        ...
                        <div class="s-acc-content" onclick="event.stopPropagation()">
                            ...
                            <div class="s-btn-group"><div class="s-btn-flat" onclick="SettingsApp.fetchModels('img')">FETCH / 获取列表</div></div></div>
                            <div class="p-list" id="img-presets"></div>
                            <div class="s-btn-group"><div class="s-btn-flat s-btn-dark" onclick="SettingsApp.saveCore('img')">SAVE / 保存</div><div class="s-btn-flat" onclick="SettingsApp.savePreset('img')">AS PRESET / 存为预设</div></div>
                        </div>
                    </div>
                    ...
                    <div class="s-accordion" onclick="SettingsApp.toggle(this)">
                        ...
                        <div class="s-acc-content" onclick="event.stopPropagation()">
                            <div class="s-footer">
                                <div class="s-btn-flat" onclick="SettingsApp.exportData()">EXPORT</div>
                                <div class="s-btn-flat" onclick="SettingsApp.importData()">IMPORT</div>
                                <div class="s-btn-flat s-btn-dark" style="grid-column: span 2; margin-top:10px; background:#FF3B30" onclick="SettingsApp.clearData()">WIPE ALL DATA</div>
                            </div>
                        </div>
                    </div>
    ...
    updateField(k, v) { 
        ...
        if(k === 'font_size') { this.applyGlobalStyles(); document.getElementById('font-label').innerText = `Size / ${v}px`; }
    },
    ...
    loadPreset(type, i) {
        ...
        this.config[`${type}_model`] = p.model; this.save();
    },
    ...
    handleFontFile(e) {
        ...
        r.onload = (ev) => {
            // 上传后立即预览
            this.config.temp_font_data = ev.target.result;
            this.config.temp_font_name = file.name.split('.')[0];
            this.config.current_font_data = ev.target.result;
            this.applyGlobalStyles();
            alert('预览中，满意请点击保存预设');
        };
    ...
    saveFontPreset() {
        if(!this.config.temp_font_data) return alert('请先上传字体文件');
        const name = prompt('字体预设名称:', this.config.temp_font_name);
        if(!name) return;
        this.config.font_presets.push({ name, data: this.config.temp_font_data });
        this.save();
        this.renderFontPresets();
    },
    ...
    loadFont(i) {
        if(i === -1) {
            this.config.current_font_data = null;
            this.config.temp_font_data = null;
        } else {
            this.config.current_font_data = this.config.font_presets[i].data;
        }
        this.save();
        this.applyGlobalStyles();
    },
    ...
    exportData() {
        ...
        a.download = `ycj_backup_${Date.now()}.json`; a.click();
    },
